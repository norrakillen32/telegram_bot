...
class ButtonHandler:
    ...
    def __init__(self, kb_searcher: KnowledgeBaseSearcher):
        ...
        self.button_mapping = {
            # Кнопки из меню
            "📦 Новая накладная": 1,  # как создать накладную на отгрузку
            "🔄 Создать УПД": 3,  # как создать универсальный передаточный документ УПД
            "🚚 ТТН для перевозки": 5,  # как создать товарно-транспортную накладную ТТН в 1С
            "📋 Копировать накладную": 7,  # как скопировать существующую накладную для нового документа
            "💳 Оплата поставщику": 31,  # как провести оплату поставщику
            "💰 Поступление от клиента": 27,  # как оформить поступление денег от клиента
            "💵 Выдача под отчет": 28,  # как выдать деньги под отчет сотруднику
            "🧾 Авансовые отчеты": 29,  # как заполнить и провести авансовый отчет
            "📑 Кассовая книга": 30,  # что такое кассовая книга и как ее вести
            "🏦 Банковские выписки": 17,  # как загрузить банковскую выписку в 1с
            "📈 Прибыль и убытки": 9,  # отчет о движении денежных средств
            "💰 Денежный поток": 9,  # отчет о движении денежных средств
            "📦 Остатки товаров": 26,  # как посмотреть остатки товаров на складе
            "👥 Дебиторская задолженность": 23,  # как посмотреть и проанализировать дебиторскую задолженность
            "📊 Продажи по периодам": 25,  # как сделать отчет продажи по периодам
            "📋 Товарооборот": 24,  # что такое товарооборот и как его считать
            "📋 Документы": 200,  # общий раздел документов (нужно добавить в базу)
            # Кнопки из других меню
            "📋 Счета": 43,  # как создать счет на оплату покупателю
            "📑 Акта": 12,  # как создать акт выполненных работ
            "📝 Договоры": 44,  # как создать договор с контрагентом
            "🏢 Организации": 47,  # как настроить и работать с несколькими организациями
            "⚙️ Настройки": 14,  # как настроить учетную политику в 1с
